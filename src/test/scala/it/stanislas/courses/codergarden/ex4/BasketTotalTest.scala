...
class BasketTotalTest extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks {
  "A BasketTotal" - {
  ...
  "A BasketTotal" - {
    "add a  two products data of x and y pound should be total, discountedTotal = x + y and discount = 0" in {
      forAll { (x: BigDecimal, y: BigDecimal) =>
        val priceX = Pound(x)
        val priceY = Pound(y)
        val basketTotal = BasketTotal().addProduct(ProductData(Product("randomProductX"), priceX))
        val productData = ProductData(Product("randomProductY"), priceY)
